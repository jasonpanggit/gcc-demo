...
Provides 8 read-only network diagnostic tools via FastMCP:
  1. virtual_network_list     — List all VNets in a subscription / resource group
  2. inspect_vnet             — VNet address space, subnets, peerings, DNS
  3. inspect_nsg_rules        — NSG security rules with traffic direction
  4. get_effective_routes     — Effective routes for a VM/NIC
  5. test_network_connectivity — Azure Network Watcher connectivity check
  6. inspect_appgw_waf        — App Gateway / WAF config and backend health
  7. inspect_vpn_expressroute — VPN Gateway / ExpressRoute circuit status
  8. check_dns_resolution     — DNS resolver configuration and name resolution
...
async def virtual_network_list(
    ...
    try:
        ...
        vnets = await loop.run_in_executor(None, _list)

        result = []
        for vnet in vnets:
            rg_name = ""
            if vnet.id:
                parts = [p for p in vnet.id.split("/") if p]
                for i, part in enumerate(parts):
                    if part.lower() == "resourcegroups" and i + 1 < len(parts):
                        rg_name = parts[i + 1]
                        break
            result.append({
                ...
                "address_prefixes": list(vnet.address_space.address_prefixes) if vnet.address_space else [],
