...
class NetworkMCPClient:
    ...
    async def _register_with_registry(self) -> None:
        ...
        try:
            ...
            logger.debug("Registered Network MCP client with tool registry")
        except ValueError as exc:
            if "already registered" in str(exc):
                logger.debug("Network MCP client already registered with tool registry")
            else:
                raise
        except Exception as exc:
            logger.warning(
                "Failed to register Network MCP client with tool registry: %s",
                exc,
                exc_info=True
            )
