"""
Software inventory cache built on top of base_cosmos.
Stores software inventory query results.
"""
import hashlib
import json
import asyncio
...
class SoftwareInventoryMemoryCache:
    def __init__(self, container_id: str = 'inventory_software'):
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.debug(f"SoftwareInventoryMemoryCache initialized with container {self.container_id}")
        except Exception as e:
            logger.debug(f"SoftwareInventoryMemoryCache initialize failed: {e}")

    def _ensure_container(self):
        """Ensure container is available, get it if needed"""
        if not self.container and base_cosmos.initialized:
            try:
                logger.debug(f"SoftwareInventoryCache: Getting container {self.container_id}")
                self.container = base_cosmos.get_container(self.container_id)
                self.initialized = True
                logger.debug(f"SoftwareInventoryCache: Container {self.container_id} obtained successfully")
            except Exception as e:
                logger.debug(f"Failed to get container {self.container_id}: {e}")
        elif self.container:
            logger.debug(f"SoftwareInventoryCache: Container {self.container_id} already available")
    ...
    def _generate_cache_key(self, agent_name: str, query_params: Dict[str, Any], workspace_id: str) -> str:
        ...
        key_data = f"software_{agent_name}_{sorted_params}_{workspace_id}".lower()
    ...
    async def get_cached_data(self, agent_name: str, query_params: Dict[str, Any], workspace_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"SoftwareInventoryMemoryCache error: {e}")
            return None
    ...
    async def cache_data(self, agent_name: str, query_params: Dict[str, Any], workspace_id: str, data: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            cached = CachedInventoryData(id=cache_key, cache_type='software', agent_name=agent_name, query_params=query_params, data=data, data_count=len(data), created_at=created.isoformat(), expires_at=expires.isoformat(), cache_key=cache_key, workspace_id=workspace_id)
            ...
            logger.debug(f"SoftwareInventoryMemoryCache cache_data error: {e}")
            return False
    ...
    async def clear_cache(self, agent_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"SoftwareInventoryMemoryCache clear_cache error: {e}")
    ...
    async def get_cache_stats(self):
        ...
        cache_key = "software_cache_stats"
        async with self.memory_lock:
        ...
        if cached_stats:
            ...
            if datetime.now(timezone.utc) <= expires_at:
                return {
                    ...
                    "type_breakdown": cached_stats.data.get("type_breakdown", {"software": 0}),
        ...
        try:
            ...
            logger.debug(f"SoftwareInventoryMemoryCache get_cache_stats error: {e}")
            return {"success": False, "error": str(e)}


# Singleton
software_inventory_cache = SoftwareInventoryMemoryCache()
