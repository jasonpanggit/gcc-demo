...
.. deprecated::
    ToolEmbedder is superseded by :class:`utils.unified_router.UnifiedRouter`
    (Phase 3 — unified routing pipeline).

    **Migration:**
    Replace ``ToolEmbedder.retrieve()`` with::

        from utils.unified_router import get_unified_router
        plan = await get_unified_router().route(query, strategy="quality")

    The "quality" strategy provides secondary-domain expansion analogous
    to semantic ranking without requiring an embedding API call.

    ToolEmbedder remains functional for backward compatibility with
