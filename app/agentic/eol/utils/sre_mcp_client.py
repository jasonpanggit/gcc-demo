    ...
    async def _register_with_registry(self) -> None:
        ...
        try:
            ...
            await registry.register_server(
                ...
                client=self,
                domain="sre",
                priority=10,  # SRE has standard priority
                auto_discover=True
            )
            logger.debug("Registered SRE MCP client with tool registry")
        except ValueError as exc:
            # Already registered - this is OK
            if "already registered" in str(exc):
                logger.debug("SRE MCP client already registered with tool registry")
            else:
                raise
        except Exception as exc:
            logger.warning(
                "Failed to register SRE MCP client with tool registry: %s",
                exc,
                exc_info=True
            )
