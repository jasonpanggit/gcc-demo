...
async def agent_communication_stream():
    ...
    def _format_event(comm: dict) -> dict:
        ...
        if evt_type == 'action':
            ...
            event_data['tool_names'] = comm.get('tool_names')
            event_data['tool_params'] = comm.get('tool_params')
            event_data['tool_details'] = comm.get('tool_details')
        elif evt_type == 'observation':
            event_data['tool_name'] = comm.get('tool_name')
            event_data['tool_result'] = comm.get('tool_result')
            event_data['is_error'] = comm.get('is_error', False)
        elif evt_type == 'pipeline_plan':
            plan = comm.get('plan') or {}
            steps = plan.get('steps') or []
            tool_names = [s.get('tool_name') for s in steps if s.get('tool_name')]
            event_data['tool_names'] = tool_names
            event_data['content'] = f"Plan: {len(steps)} step(s) — {', '.join(tool_names)}"
