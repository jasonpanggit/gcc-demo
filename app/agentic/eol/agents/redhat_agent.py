...
class RedHatEOLAgent(BaseEOLAgent):
    ...
    async def _cache_data(self, software_name: str, version: Optional[str], data: Dict[str, Any], source_url: Optional[str] = None):
        ...
        try:
            if not source_url:
                ...
                if "centos" in software_lower:
                    source_url = self.eol_urls.get("centos")
                elif "fedora" in software_lower:
                    source_url = self.eol_urls.get("fedora")
                else:
                    source_url = self.eol_urls.get("rhel", "https://access.redhat.com/support/policy/updates/errata/")
    ...
    async def purge_cache(self, software_name: Optional[str] = None, version: Optional[str] = None) -> Dict[str, Any]:
        """Purge cached data for specific software or all Red Hat cache"""
        try:
            if software_name:
