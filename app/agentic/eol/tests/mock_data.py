...
class MockDataGenerator:
    ...
    def invalid_data(self) -> List[Dict[str, Any]]:
        """
        Return a list of malformed / invalid records for edge-case and
        negative-path testing.

        Each record deliberately violates at least one schema expectation:
        wrong types, missing required keys, obviously bad values, etc.  Tests
        that verify input validation or error handling should use this data.
        """
        return [
            # Missing required 'name' field
            {"computer": "WEBSRV-EUS-001", "version": "1.0", "publisher": "Acme"},
            # 'version' is an integer instead of a string
            {"computer": "APPSRV-WUS-002", "name": "Broken App", "version": 999, "publisher": "Acme"},
            # Completely empty record
            {},
            # None values for required fields
            {"computer": None, "name": None, "version": None, "publisher": None},
            # 'last_seen' is not a valid ISO-8601 timestamp
            {
                "computer": "DBSRV-NEU-003",
                "name": "Bad Date App",
                "version": "2.0",
                "publisher": "Acme",
                "last_seen": "not-a-date",
            },
            # Extra unexpected field that parsers should tolerate or reject
            {
                "computer": "DC-WEU-004",
                "name": "Extra Fields App",
                "version": "3.0",
                "publisher": "Acme",
                "__injected__": "<script>alert(1)</script>",
            },
        ]

    def edge_case_data(self) -> List[Dict[str, Any]]:
        """
        Return records that sit at the boundary of valid input ranges.

        Useful for confirming that the application handles empty strings,
        null optional fields, and maximum-length strings without crashing.
        """
        max_str = "A" * 255  # Maximum typical VARCHAR length
        return [
            # Empty string for optional text fields
            {
                "computer": "WEBSRV-EUS-001",
                "name": "",
                "version": "",
                "publisher": "",
                "software_type": "",
            ...
            # Maximum-length strings
            {
                ...
                "install_date": None,
                "last_seen": datetime.utcnow().isoformat(),
                "computer_count": 2**31 - 1,  # INT_MAX
                "source": "log_analytics_configurationdata",
            },
            # Unicode / multi-byte characters
            {
                "computer": "FILESRV-JPN-001",
                "name": "ソフトウェア",
                "version": "日本語バージョン",
                "publisher": "株式会社テスト",
                "software_type": "Application",
    ...
    def large_dataset(self, n: int = 1000) -> List[Dict[str, Any]]:
        """
        Return a software-inventory list with *n* records for load / performance
        testing.

        Args:
            n: Total number of records to generate.  Defaults to 1 000.
                Typical values used in tests: 1 000, 5 000, 10 000.

        Returns:
            List of software inventory items in the same format as
            :meth:`generate_software_inventory`.
        """
        # Distribute records across a reasonable number of computers
        computers_needed = max(1, n // 10)
        results = self.generate_software_inventory(
            num_computers=computers_needed,
            software_per_computer_range=(10, 10),
        )
        # Trim or pad to exactly n records
        if len(results) >= n:
            return results[:n]
        # Pad by cycling through the generated records
        while len(results) < n:
            deficit = n - len(results)
            results.extend(results[:deficit])
        return results
...
def get_mock_software_inventory(num_computers: int = 50) -> Dict[str, Any]:
    ...
    """
    data = MockDataGenerator().generate_software_inventory(num_computers=num_computers)
    return {
        "success": True,
        "data": data,
        "count": len(data),
...
def get_mock_os_inventory(num_computers: int = 50) -> Dict[str, Any]:
    ...
    Returns:
        Dict with success, data, count, query_params, from_cache, cached_at
    """
    data = MockDataGenerator().generate_os_inventory(num_computers=num_computers)
    return {
        "success": True,
        "data": data,
        "count": len(data),
...
def get_mock_eol_data() -> Dict[str, Any]:
    ...
    return MockDataGenerator().generate_eol_data()
