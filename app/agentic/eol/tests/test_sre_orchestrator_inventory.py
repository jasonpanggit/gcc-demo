...
class TestResourceExistenceFastPath:
    ...
    async def test_cache_hit_sub_millisecond(self, inventory_client):
        """L1 cache hit should return a correct result without engine fallback."""
        ...
        # Correctness assertion — the timing intent is validated by
        # test_cached_lookup_faster_than_live via relative comparison.
        # Relative comparison — avoids CI flakiness
    ...
    async def test_resource_not_found_fast_path(self, inventory_client):
        """Querying a non-cached resource type should return empty without engine fallback."""
        with patch("utils.resource_inventory_client.config") as mock_config:
            mock_config.azure.subscription_id = MOCK_SUB
        ...
        # Relative comparison — avoids CI flakiness.
        # The fast-path correctness (empty list, no engine call) is the real intent.
        assert result == []
