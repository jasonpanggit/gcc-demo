...
try:
    from .mock_data import get_mock_software_inventory, get_mock_os_inventory
except ImportError:
    from mock_data import get_mock_software_inventory, get_mock_os_inventory
...
class MockSoftwareInventoryAgent:
    ...
    def __init__(self):
        ...
        logging.getLogger(__name__).debug("MockSoftwareInventoryAgent initialized (mock mode)")
    ...
    async def get_software_inventory(
        ...
        delay = float(os.environ.get("MOCK_AGENT_DELAY_MS", "0")) / 1000
        ...
        logging.getLogger(__name__).debug("[MOCK] Retrieved %d software items", result['count'])
        return result
    ...
    async def clear_cache(self) -> Dict[str, Any]:
        ...
        delay = float(os.environ.get("MOCK_AGENT_DELAY_MS", "0")) / 1000
        await asyncio.sleep(delay)
        logging.getLogger(__name__).debug("[MOCK] Cleared software inventory cache")
        return {
            "success": True,
...
class MockOSInventoryAgent:
    ...
    def __init__(self):
        ...
        self.workspace_id = "mock-workspace-id"
        logging.getLogger(__name__).debug("MockOSInventoryAgent initialized (mock mode)")
    ...
    async def get_os_inventory(
        ...
        logging.getLogger(__name__).debug("[MOCK] Retrieved %d OS items", result['count'])
        return result
    ...
    async def clear_cache(self) -> Dict[str, Any]:
        ...
        delay = float(os.environ.get("MOCK_AGENT_DELAY_MS", "0")) / 1000
        await asyncio.sleep(delay)
        logging.getLogger(__name__).debug("[MOCK] Cleared OS inventory cache")
        return {
            "success": True,
            "message": "Mock cache cleared",
            "agent": self.agent_name
