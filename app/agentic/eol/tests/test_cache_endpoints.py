...
class TestCacheEndpoints:
    ...
    async def test_get_cache_stats(self, client):
        """Test GET /api/cache/stats - Get cache statistics (endpoint removed)"""
    ...
    async def test_get_cache_size(self, client):
        """Test GET /api/cache/size - Get cache size (endpoint removed)"""
    ...
    async def test_clear_cache(self, client):
        """Test POST /api/cache/clear - Clear all cache (has implementation error)"""
        response = await client.post("/api/cache/clear")
        assert response.status_code == 200
        data = response.json()
        # Known issue: endpoint has TypeError, returns success=False
