...
async def test_vm_inventory_overview_uses_inventory_backed_fallback_for_zero_match_rows():
    ...
    service.get_vm_vulnerability_summary = AsyncMock(return_value={
        "total_cves": 78,
        "critical": 2,
        "high": 47,
        "medium": 10,
    ...
    with patch("api.cve_inventory._get_cve_vm_service", AsyncMock(return_value=service)), \
         patch("api.patch_management.list_machines", AsyncMock(return_value={
             "data": [{
                 "resource_id": vm_id,
                 "computer": "testvm1",
    ...
    service.get_vm_vulnerability_summary.assert_awaited_once_with(vm_id, allow_live_cve_fallback=False)
